...
CONNECTOR_CLASSES = [EmailDraftConnector, ChannelRecapConnector, TaskCreateConnector, CalendarFollowupConnector]
...
def test_every_connector_execute_takes_only_a_payload():
    for cls in CONNECTOR_CLASSES:
        ...
        assert len(params) == 1, f"{cls.__name__}.execute() should take exactly (payload); got {sig}"
...
def test_every_connector_has_a_kind_matching_its_purpose():
    for cls in CONNECTOR_CLASSES:
        assert hasattr(cls, "kind"), f"{cls.__name__} is missing the required `kind` class attribute"
