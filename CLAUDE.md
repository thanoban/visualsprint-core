...
- **Agents run on Claude via Vertex AI by default**, not the direct Anthropic API. `LlmClient` adapter targets `anthropic[vertex]` / Vertex AI's Claude endpoints; auth is GCP service-account credentials (same GCP project as the Google Speech-to-Text vendor, so one set of cloud credentials covers both). Model IDs stay the same (`claude-sonnet-5` etc.) — only the transport changes.
  ...
  - **Update 2026-08-11 — default provider is now Gemini, not Claude.** Verified empirically against the live `visualsprint-agent` GCP project: Anthropic's partner models on Vertex default new projects to **zero quota per base model** (`aiplatform.googleapis.com/global_online_prediction_requests_per_base_model`, `anthropic-claude-sonnet` bucket), requiring a manual quota-increase request before any call succeeds — confirmed via a live 429 `RESOURCE_EXHAUSTED` response, not assumed. Gemini is Google's first-party model on Vertex and carries no such partner-quota gate; `gemini-2.5-pro` / `gemini-2.5-flash-lite` were verified live (text, structured JSON via native `response_schema`, and vision) with zero friction. User explicitly directed the switch given the budget-constrained (student, single GCP trial credit) context. `app/adapters/llm_gemini_vertex.py` (`GeminiVertexLlmClient`) is now the default (`VS_LLM_PROVIDER=gemini|vertex|foundry`, default `gemini`) — Claude-on-Vertex and Claude-on-Foundry remain available, not deleted, in case the quota is granted later or a specific stage needs Claude specifically. Structured output no longer needs the forced-tool-use workaround — Gemini supports schema-constrained JSON generation natively (a Pydantic class passed directly as `response_schema`).
