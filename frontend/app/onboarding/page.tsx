...
function Card({
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 13.5, lineHeight: 1.55, color: "var(--text-muted)", margin: "0 0 16px" }}>{desc}</p>
      <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 8 }}>
        {items.map((item) => (
          <li key={item} style={{ fontSize: 13, color: "var(--text-muted)", display: "flex", gap: 8 }}>
    ...
    <div style={{ background: "var(--bg)", color: "var(--text)", fontFamily: sans, minHeight: "100vh" }}>
      <header style={{ padding: "20px 32px", borderBottom: "1px solid var(--border)" }}>
        <a href="/welcome" style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontFamily: mono, color: "var(--accent)", fontWeight: 600 }}>[</span>
      ...
      <main style={{ maxWidth: 720, margin: "0 auto", padding: "64px 32px 80px" }}>
        <p style={{ fontFamily: mono, fontSize: 12.5, fontWeight: 600, letterSpacing: "0.03em", textTransform: "uppercase", color: "var(--accent-strong)", margin: "0 0 12px" }}>
          Set up your workspace
        </p>
        <h1 style={{ fontFamily: serif, fontSize: 34, color: "var(--text)", margin: "0 0 12px" }}>Who&apos;s this for?</h1>
        <p style={{ fontSize: 15, lineHeight: 1.6, color: "var(--text-muted)", margin: "0 0 36px", maxWidth: 520 }}>
        ...
        <div style={{ marginTop: 36 }}>
          ...
          <p style={{ fontSize: 12.5, color: "var(--text-faint)", margin: "14px 0 0" }}>
